...
namespace Katas
{
    ...
    {
        private static int _files_count;
        private static string _fileName;

        public static string Match(string pattern, string flags, string[] files)
        {
            _files_count = files.Length;
            string returnString = "";
            Directory.SetCurrentDirectory(Path.GetTempPath());
            foreach (var filename in files)
            {
                ...
                for (int index = 0; index < content.Length; index++)
                {
                    if (flags != "")
                    {
                        if (CaseInsensitiveMatch(flags))
                        {
                            content = content.Select(s => s.ToLowerInvariant()).ToArray();
                            pattern = pattern.ToLower();

                            if (!ExactMatch(flags) && !PrintOnlyFileName(flags))
                            {
                                if (content[index].Contains(pattern))
                                {
                                    returnString += "\n" + GetReturnString(flags, index, contentCopy);
                                }
                            }
                        }

                        if (PrintOnlyFileName(flags))
                        {
                            if (ExactMatch(flags))
                            {
                            ...
                            else if (content[index].Contains(pattern))
                            {
                                returnString += "\n" + filename;
                                break;
                            }
                        ...
                        if (InvertedMatch(flags))
                        {
                            if (ExactMatch(flags))
                            {
                                if (!(content[index] == pattern))
                                {
                                    returnString += "\n" + GetReturnString(flags, index, content);
                                }
                            }
                        ...
                        if (ExactMatch(flags) && !PrintOnlyFileName(flags) && !InvertedMatch(flags))
                        {
                            if (content[index] == pattern)
                            {
                                returnString += "\n" + GetReturnString(flags, index, contentCopy);
        ...
        private static bool CaseInsensitiveMatch(string flags)
        {
            return flags.Contains("-i");
        }

        private static bool InvertedMatch(string flags)
        {
            return flags.Contains("-v");
        }

        private static bool PrintOnlyFileName(string flags)
        {
            return flags.Contains("-l");
        }

        private static bool ExactMatch(string flags)
        {
            return flags.Contains("-x");
        }

        private static string GetReturnString(string flags, int index, string[] content)
        {
            string return_string = "";

            if (PrintLineNumbers(flags))
                return_string = (index + 1).ToString() + ":" + content[index];
            else
                return_string = content[index];

            if (_files_count > 1)
                return_string = _fileName + ":" + return_string;

            return return_string;
        }

        private static bool PrintLineNumbers(string flags)
        {
